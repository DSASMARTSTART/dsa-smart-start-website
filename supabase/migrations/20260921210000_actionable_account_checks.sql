-- Archived profiles are retained for history, not counted as broken live accounts.
BEGIN;
CREATE OR REPLACE FUNCTION public.admin_analytics_snapshot(p_days integer DEFAULT 30,p_currency text DEFAULT NULL,p_timezone text DEFAULT 'Europe/Belgrade')
...
BEGIN
 ...
 'health',jsonb_build_object(
   ...
   'profilesWithoutAccount',(SELECT count(*) FROM users u WHERE u.status<>'deleted' AND NOT EXISTS(SELECT 1 FROM auth.users a WHERE a.id=u.id)),
